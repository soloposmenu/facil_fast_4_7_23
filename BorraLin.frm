...
If txtLin = "" Or Not IsNumeric(txtLin) Or Val(txtLin) > CajLin Then
    ShowMsg "Numero de Linea NO es VALIDO", vbBlue, vbYellow
Else
    ...
    If rsFixTmpTrans.EOF = True Then
        'INFO: 13DIC2012
        ShowMsg "NUMERO DE LINEA, NO EXISTE", vbBlue, vbYellow
        rsFixTmpTrans.Close
    ...
    If rsFixTmpTrans!CANT < 0 Then
        ShowMsg "El Producto ya fue ANULADO", vbBlue, vbYellow
        rsFixTmpTrans.Close
        Set rsFixTmpTrans = Nothing
        Exit Sub
    End If
    ...
    If nLinDescuento > 0 Then
        ...
        'msConn.Execute "UPDATE TMP_TRANS SET TIPO = '  ' WHERE MESA = " & nMesa & " AND CAJERO = " & npNumCaj & " AND LIN = " & nLinDescuento
        'UPDATE: 26ABR2024
        cSQLFix = "UPDATE TMP_TRANS SET TIPO = '  ', FE_DESCUENTO = 0 "
        cSQLFix = cSQLFix & " WHERE MESA = " & nMesa
        cSQLFix = cSQLFix & " AND CAJERO = " & npNumCaj
        cSQLFix = cSQLFix & " AND LIN = " & nLinDescuento
    ...
    'LOG DE ANULACION
    'INFO: NO SE HABIA CREADO HASTA AHORA. 26SEP2017
    EscribeLog "ANULACION " & RegRead("HKCU\Software\SoloSoftware\SoloMix\LastAuthorization") & ". " & " (" & rsFixTmpTrans!CANT & ") " & "VO-" & rsFixTmpTrans!DESCRIP & ", valor: " & Format(rsFixTmpTrans!precio * (-1), "CURRENCY")
...
'AQUI NO APLICA MUESTRAPRODUCTOSMARCADOS
    rs07.Open "SELECT a.lin,a.descrip,a.cant," & " format(precio_unit,'##0.00') as mPrecio_unit," & " format(precio,'##0.00') as mPrecio," & " a.cajero,a.mesero,a.depto,a.plu,a.mesa,a.valid, " & " a.envase,a.fecha,a.hora,a.tipo,a.descuento,a.cuenta, " & " a.caja " & " FROM TMP_TRANS as a " & " WHERE a.MESA = " & nMesa & " AND a.CAJERO = " & npNumCaj & " ORDER BY a.lin", msConn, adOpenStatic, adLockReadOnly
    ...
    rsParciales.Open "SELECT MESA,SUM(MONTO) AS VALOR FROM TMP_PAR_PAGO " & " WHERE MESA = " & nMesa & " GROUP BY MESA", msConn, adOpenDynamic, adLockOptimistic
    ...
    If lParc = 1 Then
        PLU.PlatosMesa.AddItem (CajLin + 1) & Chr(9) & "PAGO.PARCIAL" & Chr(9) & 1 & Chr(9) & Format(rsParciales!Valor, "STANDARD") & Chr(9) & Format(rsParciales!Valor, "STANDARD")
